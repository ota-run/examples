...
- multi-workflow repos often need more than one readiness lane
- archived receipts are only trustworthy when the selected workflow stays explicit
- `latest` and `promoted` should not silently cross from one workflow history into another

Try this:

```bash
ota receipt --workflow verify --json --archive
ota receipt --workflow app --json --archive
ota receipt --workflow verify --json --baseline latest
ota receipt --workflow verify --snapshot latest
```
